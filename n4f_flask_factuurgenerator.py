...
from reportlab.lib.utils import ImageReader, simpleSplit, simpleSplit
import qrcode
  ...
  <div class="wrap">
    <aside class="panel">
      ...
      <form method="post" action="/generate-pdf">
        ...
        <div class="field"><label>Adres</label><textarea name="businessAddress" placeholder="Straat + huisnummer&#10;Postcode Plaats"></textarea></div>
...
def generate_pdf() -> Response:
    ...
    qr_x = 138 * mm
    # vaste veilige positie onderaan pagina zodat QR nooit afgesneden wordt
    page_height = A4[1]
    qr_y = page_height - 80 * mm
    c.setFillColor(WHITE)
    c.setStrokeColor(CHAMPAGNE)
    c.roundRect(qr_x, qr_y - 32 * mm, 58 * mm, 62 * mm, 4 * mm, stroke=1, fill=1)
    qr_image = ImageReader(qr_buffer)
    c.drawImage(qr_image, qr_x + 9 * mm, qr_y - 14 * mm, width=36 * mm, height=36 * mm, mask='auto')
    c.setFillColor(BLACK)
    c.setFont("Helvetica-Bold", 10)
